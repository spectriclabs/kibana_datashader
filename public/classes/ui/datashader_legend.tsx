...
export class DatashaderLegend extends React.Component<Props, State> {
  ...
  async _setStateFromGlobal() {
    ...
    if (
      DATASHADER_BUCKET_SELECT[descriptor.id] &&
      DATASHADER_BUCKET_SELECT[descriptor.id].toString() !== this.state.bucketRange.toString()
    ) {
  ...
  async _loadLegendInfo() {
    ...
    if (dataMeta.sourceQuery) {
      if (dataMeta.sourceQuery.language === 'kuery') {
        const kueryNode = fromKueryExpression(dataMeta.sourceQuery.query);
        const kueryDSL = toElasticsearchQuery(kueryNode);
      ...
      } else if (dataMeta.sourceQuery.language === 'lucene') {
        const luceneDSL = luceneStringToDsl(dataMeta.sourceQuery.query);
        currentParamsObj.filters.push({
          meta: {
            type: 'bool',
          },
  ...
  render() {
    ...
    const bucketOnChange = debounce((v: [number, number]) => {
      ...
      const map = this.props.style.getMap();
      if (map) {
          ...
          <EuiFlexGroup direction={'row'} gutterSize={'none'}>
            ...
            <EuiFlexItem grow={false}>
              <svg
                ...
                height="16"
                viewBox="0 0 16 16"
                style={{ stroke: config.color, fill: config.color }}
              >
